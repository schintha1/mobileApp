...
public class shop extends Fragment {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.isChecked()){
            ...
            checkString.remove(item.getTitle().toString());
            item.setChecked(false);
        }
        else {
            checkString.add(item.getTitle().toString());
    ...
    public class DB extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            for (int i = 0; i <checkString.size(); i++){
                if (checkString.get(i).equals("Amazon")){

                    String queryString = "select * from Product where idVendor =1";
                    try {
                ...
                else if (checkString.get(i).equals("Walmart")){

                    String queryString = "select * from Product where idVendor =3";
                    try {
                ...
                else if (checkString.get(i).equals("Kroger")){
        ...
        protected void onPostExecute(Void result) {
            ...
            for (int i=0 ;i < checkItem.size(); i++){
                ...
                /*ImageView tempv = new ImageView(getActivity());
                if (checkItem.get(i).getVendor().equals("Amazon")){
                    tempv.setBackgroundResource(R.drawable.amazon);
                }
                else if (checkItem.get(i).getVendor().equals("Kroger")){
                    tempv.setBackgroundResource(R.drawable.kroger);
                }
                else {
                    tempv.setBackgroundResource(R.drawable.walmart);
                }*/
                // tempbox.setR
            ...
            try {
                ...
                for (int i = 0; i<CheckBoxes.size(); i++){
                    if (CheckBoxes.get(i).isChecked()){
                      ...
                      //  if (num <=count ){
                        if(num>0) {
                            String queryString = "INSERT INTO cart Values (DEFAULT," + 1 + "," + id + ",NULL," + num + ")";
                            st.executeUpdate(queryString);
                            Toast.makeText(getActivity(),"Items are added to cart" , Toast.LENGTH_SHORT).show();
                        }
                       // }
                        /*else {
                            int cartnum = count;
                            int nisnum = num -count;
                            String queryString = "INSERT INTO cart Values (DEFAULT,"+1+","+id+",NULL,"+cartnum+")";
                            String queryString2= "INSERT INTO not_in_stock Values (DEFAULT,"+1+",NULL,NULL,"+id+","+nisnum+",NULL)";
                            st.executeUpdate(queryString);
